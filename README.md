...
# Overview 
This project analyzes how pretrained audio encoders represent speech variation during tense dialogue. Using selected scenes from movies (Weapons, Bugonia, and Blue Valentine), I tracked and attempted to mathematically quantify how, in conversations, peoples' tone and prosodic feautures evolve, respond to each other, and mirror each other. The goal was to examine whether these changes in embeddings could align with subjective perceptions. 

All results in weapons.ipynb, bugonia.ipynb, and blueval.ipynb. All functions used are in trajectory_analysis.py

# Ethical Concerns 
This project intentionally avoids real-world personal data and only uses fictional, public film dialogue. The intention should not step over the line into gauging people's worth, personality, and their entire lives, just the acoustic patterns in a moment in time, a tense moment at that too. This analysis and other adjacents are relatively rudimentary and it's completely unfair to assume ability to infer traits. People are incredibly complex and I don't intend any work to compete with intuition, rather to learn more about ourselves at best. Speech patterns are shaped by context, dialogue, narrative, performance, upbringing, and so so much more. 
...
3. Could there be any degree of "objectiveness" in how we speak that we didn't have the tools to realize and learn from before, but now we can with transformers?
